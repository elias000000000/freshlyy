...
/* ════════════════════════════════════════════════════════════════════════
   HOME TAB
════════════════════════════════════════════════════════════════════════ */
function HomeTab({items,soon,setTab,setModal}){
  const daily=RECIPE_DB[getDailyIndex(RECIPE_DB.length)];
    ...
    <div className="fu">
      ...
      <div style={{padding:"0 20px"}}>
        ...
        {sorted.map((r,i)=>{
          ...
          return(<div key={r.id} onClick={()=>setModal({type:"viewRecipe",data:r})} className="fu tap" style={{animationDelay:`${i*0.04}s`,background:C.card,border:`1px solid ${noMissing?C.greenM:C.border}`,borderRadius:16,padding:"16px",marginBottom:10,cursor:"pointer",boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div style={{fontWeight:800,fontSize:15,flex:1,paddingRight:8}}>{r.name}</div>
              {badge&&<div style={{background:`${badgeCol}15`,border:`1px solid ${badgeCol}30`,borderRadius:20,padding:"3px 9px",flexShrink:0}}><span style={{fontSize:11,fontWeight:700,color:badgeCol}}>{badge}</span></div>}
            </div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
  ...
  return(
    <div style={{fontFamily:"'Outfit',sans-serif",background:C.bg,minHeight:"100dvh",maxWidth:430,margin:"0 auto",position:"relative",overflowX:"hidden",color:C.text}}>
      ...
      <main style={{paddingBottom:88}}>
        {tab==="home"    &&<HomeTab    items={items} soon={soon} setTab={setTab} setModal={setModal}/>}
